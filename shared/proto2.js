...
const serviceProtoPath = path.join(protoRoot, "ServiceTypes_AuxSbox", "Svc_UtilityService.proto");
...
{
  // 配置文件要求使用 http://ip:port，grpc-js 实际连接前要去掉 scheme。
  return smartboxTarget.replace(/^https?:\/\//i, "");
}

function createUtilityClient(smartboxTarget)
{
  const grpcTarget = normalizeSmartboxTarget(smartboxTarget);

  return new protoDescriptor.smt_iot.v1.aux.UtilityService(
    grpcTarget,
