...
$(function(){
    ...
    $("#ToolUsageText").html(ToolUsage_L[Language])
    $("#ProgramUsageText").html(ProgramUsage_L[Language])
    $("#ConnectText").html(Connect_L[Language])
     $("#RunningTimeText").html(RunningTime_L[Language])
     $("#CuttingTimeText").html(CuttingTime_L[Language])
     $("#AlarmTimeText").html(AlarmTime_L[Language])
     $("#RunningText").html(Running_L[Language])
     $("#AlarmText").html(Alarm_L[Language])     
     $("#AlarmsText").html(Alarm_L[Language])
     $("#PeakLoadTimeText").html(PeakLoadTimeText[Language])
    $("#CoordinateChangeText").html(CoordinateChangeText[Language])
    $("#ToolChangeText").html(ToolChangeText[Language])
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!selectedOption && dropdown.options.length > 1) {
        // Priority 4: Default to the first option (skip the placeholder)
      ...
      if (selectedOption && selectedOption.value) {
        ...
        if (selectedOption.dataset) {
          ...
          sessionStorage.setItem('selectedPort', selectedOption.dataset.port);
          sessionStorage.setItem('selectedType', selectedOption.dataset.type);
          console.log(`Session Storage set: ip=${selectedOption.dataset.ip}, port=${selectedOption.dataset.port}, type=${selectedOption.dataset.type}`);
        }
      } else {
        console.error("No valid dropdown selection found.");
      }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 华兴 新加开始 调整Start-YMD时也调End-YMD的显示
...
var LeftOne =()=>{
    $.ajax({
        ...
        success: function (data) {
            if(data=="2"){
                console.log("QueryEfficiency 未返回数据")
                return
            }
            if(data.EfficiencyOutput.length>0){
                for(var i=0;i<data.EfficiencyOutput.length;i++){
                    if (data.EfficiencyOutput[i].Name == "Connected") {
                        var Time = data.EfficiencyOutput[i].Value[0].toFixed(2) * 100;
                        $("#barValue1").css("width",parseInt(Time)+"%");
                        $("#PercentValue1").html(parseInt(Time)+"%")
                    }
                    if (data.EfficiencyOutput[i].Name == "UpTime") {
                        var Time = data.EfficiencyOutput[i].Value[0].toFixed(2) * 100;
                        $("#barValue2").css("width",parseInt(Time)+"%");
                        $("#PercentValue2").html(parseInt(Time)+"%")
                    }
                    if (data.EfficiencyOutput[i].Name == "Cutting") {
                        var Time = data.EfficiencyOutput[i].Value[0].toFixed(2) * 100;
                        $("#barValue3").css("width",parseInt(Time)+"%");
                        $("#PercentValue3").html(parseInt(Time)+"%")
                    }
                    if (data.EfficiencyOutput[i].Name == "Alarm") {
                        var Time = data.EfficiencyOutput[i].Value[0].toFixed(2) * 100;
...
var Left234=()=>{
    $.ajax({
        url: "/router/use",      
        data:{
            ip: getSelectedIp("QueryUsagePeriod"), // 华兴 更改
            port: getSelectedPort("QueryUsagePeriod"), // 华兴 更改
            start:Stime,
            end:Etime,
            type:'QueryUsagePeriod'
        },
        type: "get",          
        datatype: "json",
        // 华兴 新加开始
        ...
        success: function (data) {
            if(data=="2"){
                console.log("QueryUsagePeriod 未返回数据")
                return
            }
            if($.isEmptyObject(data)==true){
                console.log("QueryUsagePeriod 返回空数据")
                return
            }
            if(data.UsageType.length>0){
                for(let i=0;i<data.UsageType.length;i++){
                    if(data.UsageType[i].Name=="Tool"){
                        let CountTool = 1;
                        let Tool = "";
                        let Time = "";
                        $("#left_3_main").html("");
                        for (var j = 0; j < data.UsageType[i].Value.length; j++) {
                            if (CountTool == 1) {
                                Tool = data.UsageType[i].Value[j];
                                Time = data.UsageType[i].Value[j + 1];
                                //console.log(Tool,Time)
                                $("#left_3_main").append("<div><span>"+Tool+"</span><span>"+(Time / 60).toFixed(1)+" min</span></div>");
                                CountTool++;
                            }
                            else if (CountTool == 2) {
                                CountTool = 1;
                    ...
                    if(data.UsageType[i].Name=="NCProgram"){
                        let CountNC = 1;
                        let NcName = "";
                        let NcTime = "";
                        let Ncfreq = "";
                        $("#left_4_main").html("")
                        for (let j = 0; j < data.UsageType[i].Value.length; j++) {
                            if (CountNC == 1) {
                                NcName = data.UsageType[i].Value[j];
                                NcTime = data.UsageType[i].Value[j + 1];
                                Ncfreq = data.UsageType[i].Value[j + 2];
                                if (NcName.indexOf('/') > -1) {
                                    NcName = NcName.split('/')[NcName.split('/').length-1];
                                }
                                $("#left_4_main").append("<div><span>" + NcName + "</span><span>" + (NcTime / 60).toFixed(1) + " min</span><span>" + Ncfreq + " freq</span></div>")
                    ...
                    if(data.UsageType[i].Name=="MachineMode"){
                        if($("#left_2").length==0){
                            return;
                        }
                        let modeData = [];
                        ...
                        for (let j = 0; j < data.UsageType[i].Value.length; j++) {
                            ...
                            if (CountM == 1) {
                                ModeN = data.UsageType[i].Value[j];
                                Mtime = data.UsageType[i].Value[j + 1];
                                var mm = {
                                    "Time": Mtime,
                                    "Mode": ModeN
                                }
                        ...
                        for (let j = 0; j < modeData.length; j++) {
                            Mode.push(modeData[j].Mode);
                        }
                        Mode = new Set(Mode);//去重复
                        const ModeArr = [...Mode];
                        var newModeDate = [];
                        for (let ii = 0; ii < ModeArr.length; ii++) {
                            var sum = 0
                            for (let jj = 0; jj < modeData.length; jj++) {
                                if (ModeArr[ii] == modeData[jj].Mode) {
                                    sum += parseInt(modeData[jj].Time)
                                }
                                var tit2 = {
                                    "Time": sum,
                                    "Mode": ModeArr[ii]
                            ...
                            newModeDate.push(tit2);
                        }
                        $("#left_2_main").html("");
                        for (let z = 0; z < newModeDate.length; z++) {
                          // 华兴 更改
                          $("#left_2_main").append("<div><span>" + Machine_Mode_Name(newModeDate[z].Mode, getSelectedType("Left234")) + " </span><span>" + (newModeDate[z].Time / 60).toFixed(1) + " min</span></div>")
                        }
        ...
        complete: function (XHR, TS) {
            XHR = null;
            if(Tag){
                SetTimeout2 = self.setTimeout(function(){
                    if($("#left_2").length==0&&$("#left_3").length==0&&$("#left_4").length==0){
                        //alert(1)
                    }
                    else{
                        Left234();
...
var Left6=()=>{
    $.ajax({
        url: "/router/use",      
        data:{
            ip: getSelectedIp("QueryHistoricalBar"), // 华兴 更改
            port: getSelectedPort("QueryHistoricalBar"), // 华兴 更改
            start:Stime,
            end:Etime,
            type:'QueryHistoricalBar'
        },
        type: "get",          
        datatype: "json",
        // 华兴 新加开始
        ...
        success: function (data) {
            var ChartData=data
            $("#left_6_main").html("")
            if($.isEmptyObject(ChartData)==false){
                if(typeof ChartData.HistoricalBar!== 'undefined'){
                    for (var i = 0; i < ChartData.HistoricalBar.length; i++) {
                        var AlarmData = [];
                        if (ChartData.HistoricalBar[i].Name == "Alarm") {
                            $("#AlarmSum").html(ChartData.HistoricalBar[i].Value.length/3)
                            var countA = 1;
                            var AlarmName = "";
                            var time="";
                            for (let j = 0; j < ChartData.HistoricalBar[i].Value.length; j++) {
                                if (countA == 1) {//开始时间
                                    var starttime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), '7', ChartData.HistoricalBar[i].Value[j + 2]];
                                    time=ChartData.HistoricalBar[i].Value[j]
                                    AlarmData.push(starttime)
                                    var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j + 1])), '7', ChartData.HistoricalBar[i].Value[j + 2]];
                                    AlarmData.push(endtime)
                                    countA++;
                                    continue;
                                }
                                else if (countA == 2) {//结束时间
                                    var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), ''];
                                    AlarmData.push(endtime)
                                    countA++;
                                    continue
                                }
                                    ...
                                    countA = 1;
                                    AlarmName = ChartData.HistoricalBar[i].Value[j];
                                }
                                $("#left_6_main").append('<div><img src="./img/icon_warning_2.png" alt=""><span>'+AlarmName+'</span><span>'+ConvertDateFromUtcToLocal(time)+'</span></span>');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for(var x=0;x<data.length;x++){
                var ChartData=data[x];
                if($.isEmptyObject(ChartData)==false){
                    if(typeof ChartData.HistoricalEvent!== 'undefined'){
                        for (let i = 0; i < ChartData.HistoricalEvent.length; i++) {
                            if (ChartData.HistoricalEvent[i].Name == "StartNCProgram") {
                                var countNC = 1;
                                for (let j = 0; j < ChartData.HistoricalEvent[i].Value.length; j++) {
                                    if (countNC == 1) {
                                        var ly = {
                                            name: "" + ChartData.HistoricalEvent[i].Value[j + 1] + "",
                                            type: 'line',
                                            step: 'end',
                                            data: [[new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalEvent[i].Value[j])), 11.5]],
                                            itemStyle: {
                                                normal: {
                                                    color: "#27f7d5"
                                                }
                            ...
                            if (ChartData.HistoricalEvent[i].Name == "ToolSwap") {
                                var countTool = 1;
                                for (let j = 0; j < ChartData.HistoricalEvent[i].Value.length; j++) {
                                    if (countTool == 1) {
                                        var lyT = {
                                            ...
                                            type: 'line',
                                            step: 'end',
                                            data: [[new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalEvent[i].Value[j])), 9.5]],
                                            itemStyle: {
                                                normal: {
                                                    color: "#ed7e0a"
                                                }
                            ...
                            if (ChartData.HistoricalEvent[i].Name == "ModeChange") {
                                   var countM = 1;
                                   var outputArry=convertToTimeRanges(ChartData.HistoricalEvent[i].Value);
                                   var dataMode = outputArry;
                                   if(dataMode.length>0){
                                    dataMode = echarts.util.map(dataMode, function (item, index) {
                                        var color = "";
                                        color=MachineModeStatus(item[3]);
                                        // if (item[3] == "MDI") {
                ...
                if($.isEmptyObject(ChartData)==false){
                    if(typeof ChartData.HistoricalBar!== 'undefined'){
                        for (var i = 0; i < ChartData.HistoricalBar.length; i++) {
                            var RunData2 = [];
                            if (ChartData.HistoricalBar[i].Name == "Running") {
                                var count = 1;
                                for (let j = 0; j < ChartData.HistoricalBar[i].Value.length; j++) {
                                    if (count == 1) {//开始时间
                                        var starttime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), '7'];
                                        RunData2.push(starttime)
                                        var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j + 1])), '7'];
                                        RunData2.push(endtime)
                                        count++;
                                        continue;
                                    }
                                    else if (count == 2) {//结束时间
                                        var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), ''];
                                        RunData2.push(endtime)
                                        //var starttime = [new Date(ChartData.HistoricalBar[i].Value[j + 2]), ''];
                                        //RunData2.push(starttime)
                                        count++;
                                        continue
                                    }
                            ...
                            var FeedrateData = []
                            if (ChartData.HistoricalBar[i].Name == "Feedrate") {
                                var countF = 1;
                                var countJR = "";
                                for (let j = 0; j < ChartData.HistoricalBar[i].Value.length; j++) {
                                    if (countF == 1) {
                    
                                        var starttime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), '5'];
                                        FeedrateData.push(starttime)
                                        ...
                                        var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), '4.5'];
                                        FeedrateData.push(endtime)
                                        countF++;
                                        continue;
                            ...
                            var SpindleData = [];
                            if (ChartData.HistoricalBar[i].Name == "Spindle") {
                             var countS = 1;
                                for (let j = 0; j < ChartData.HistoricalBar[i].Value.length; j++) {
                                    if (countS == 1) {
                                        var starttime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), '2.5'];
                                        SpindleData.push(starttime)
                                        countS++;
                                        continue;
                                    }
                                    else if (countS == 2) {
                                        var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), '2'];
                                        SpindleData.push(endtime)
                                        countS++;
                                        continue;
                                    }
                                    else if (countS == 3) {
                            ...
                            if (ChartData.HistoricalBar[i].Name == "Alarm") {
                                ...
                                for (let j = 0; j < ChartData.HistoricalBar[i].Value.length; j++) {
                                    if (countA == 1) {//开始时间
                                        ...
                                        var endtime = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalBar[i].Value[j])), ''];
                                        AlarmData.push(endtime)
                                        //var starttime = [new Date(ChartData.HistoricalBar[i].Value[j + 2]), ChartData.HistoricalBar[i].Value[j + 1]];
                                        //AlarmData.push(starttime)
                                        countA++;
                                        continue
                                    }
                ...
                if($.isEmptyObject(ChartData)==false){
                    if(typeof ChartData.HistoricalGraph!== 'undefined'){
                        for (let ii = 0; ii < ChartData.HistoricalGraph.length; ii++) {
                            var FeedrateOverrideData = [];
                            if (ChartData.HistoricalGraph[ii].Name == "FeedrateOverride") {
                                var countFO = 1;
                                for (let j = 0; j < ChartData.HistoricalGraph[ii].Value.length; j++) {
                                    if (countFO == 1) {
                                        var  FO=0;
                                        FO=ChartData.HistoricalGraph[ii].Value[j + 1];
                                        if(FO<0){
                                            FO=0;
                                        }
                                        var foDate = [new Date(ConvertDateFromUtcToLocal(ChartData.HistoricalGraph[ii].Value[j])), FO / 150];
                                        FeedrateOverrideData.push(foDate);
                                        countFO++;
                                        continue;
                                    }
                                    else if (countFO == 2) {
...
var Table589=()=>{
    $.ajax({
        url: "/router/use",      
        data:{
            ip: getSelectedIp("QueryChangeRecord"), // 华兴 更改
            port: getSelectedPort("QueryChangeRecord"), // 华兴 更改
            start:Stime,
            end:Etime,
            type:'QueryChangeRecord'
        },
        type: "get",          
        datatype: "json",
        // 华兴 新加开始
        ...
        success: function (ChartData) {
            if(ChartData=="2"){
                console.log("QueryChangeRecord 未返回数据")
                return
            }
            if($.isEmptyObject(ChartData)==true){
                console.log("QueryChangeRecord 返回空数据")
                return
            }
            for (let i = 0; i < ChartData.ChangeRecord.length; i++) {
                if (ChartData.ChangeRecord[i].Name == "Peakload") {
                    var CountPL = 1;
                    $("#left_5_box table tr").slice(1).remove();
                    for (let j = 0; j < ChartData.ChangeRecord[i].Value.length; j++) {
                        ...
                        let S_Load = "";
                        if (CountPL == "1") {
                            PTIME = ConvertDateFromUtcToLocal(ChartData.ChangeRecord[i].Value[j]).split(' ')[1];
                            X_Load = round(ChartData.ChangeRecord[i].Value[j + 1])
                            Y_Load = round(ChartData.ChangeRecord[i].Value[j + 2]);
                            Z_Load = round(ChartData.ChangeRecord[i].Value[j + 3]);
                            A_Load = round(ChartData.ChangeRecord[i].Value[j + 4]);
                            S_Load = round(ChartData.ChangeRecord[i].Value[j + 5]);
                            CountPL++;
                            $("#left_5_box table").append("<tr><td>"+X_Load+"</td><td>"+Y_Load+"</td><td>"+Z_Load+"</td><td>"+A_Load+"</td><td>"+S_Load+"</td><td>"+PTIME+"</td></tr>")
                            continue;
                        }
                        if (CountPL == 6) {
                            CountPL = 1;
                ...
                if (ChartData.ChangeRecord[i].Name == "CoordChange") {
                    let CountCR = 1;
                    $("#right_8_box table tr").slice(1).remove();
                    for (let j = 0; j < ChartData.ChangeRecord[i].Value.length; j++) {
                        let GNAME = "";
                        let GTIME = "";
                        let GX = "";
                        ...
                        let LGA = "";
                        if (CountCR == "1") {
                            GNAME = ChartData.ChangeRecord[i].Value[j];
                            GTIME = ConvertDateFromUtcToLocal(ChartData.ChangeRecord[i].Value[j + 1]).split(' ')[1];
                            GX = round(ChartData.ChangeRecord[i].Value[j + 2])
                            GY = round(ChartData.ChangeRecord[i].Value[j + 3]);
                            GZ = round(ChartData.ChangeRecord[i].Value[j + 4]);
                            GA = round(ChartData.ChangeRecord[i].Value[j + 5]);
                            LGX = round(ChartData.ChangeRecord[i].Value[j + 6]);
                            LGY = round(ChartData.ChangeRecord[i].Value[j + 7]);
                            LGZ = round(ChartData.ChangeRecord[i].Value[j + 8]);
                            LGA = round(ChartData.ChangeRecord[i].Value[j + 9]);
                            CountCR++;
                            $("#right_8_box table").append("<tr><td>"+GNAME+"</td><td>"+LGX+"</td><td>"+LGY+"</td><td>"+LGZ+"</td><td>"+LGA+"</td><td>"+GX+"</td><td>"+GY+"</td><td>"+GZ+"</td><td>"+GA+"</td><td>"+GTIME+"</td><tr>");
                            continue;
                        }
                        if (CountCR == 10) {
                            CountCR = 1;
                ...
                if (ChartData.ChangeRecord[i].Name == "ToolCompensationData") {
                    let CountTC = 1;
                    $("#right_9_box table tr").slice(1).remove();
                    for (let j = 0; j < ChartData.ChangeRecord[i].Value.length; j++) {
                        let Tool = "";
                        let tTIME = "";
                        let tcD = "";
                        ...
                        let LtwH = "";
                        if (CountTC == "1") {
                            Tool = ChartData.ChangeRecord[i].Value[j];
                            tTIME = ConvertDateFromUtcToLocal(ChartData.ChangeRecord[i].Value[j + 1]).split(' ')[1];
                            tcD = round(ChartData.ChangeRecord[i].Value[j + 2])
                            twD = round(ChartData.ChangeRecord[i].Value[j + 3]);
                            tcH = round(ChartData.ChangeRecord[i].Value[j + 4]);
                            twH = round(ChartData.ChangeRecord[i].Value[j + 5]);
                            LtcD = round(ChartData.ChangeRecord[i].Value[j + 6]);
                            LtwD = round(ChartData.ChangeRecord[i].Value[j + 7]);
                            LtcH = round(ChartData.ChangeRecord[i].Value[j + 8]);
                            LtwH = round(ChartData.ChangeRecord[i].Value[j + 9]);
                            CountTC++;
                            $("#right_9_box table").append("<tr><td>"+Tool+"</td><td>"+LtcD+"</td><td>"+LtwD+"</td><td>"+LtcH+"</td><td>"+LtwH+"</td><td>"+tcD+"</td><td>"+twD+"</td><td>"+tcH+"</td><td>"+twH+"</td><td>"+tTIME+"</td></tr>")
                            //$(".ToolCompensation_box_inf").append('<div><span>' + Tool + '</span><span>' + LtcD + '</span><span>' + LtwD + '</span><span>' + LtcH + '</span><span>' + LtwH + '</span> <span>' + tcD + '</span><span>' + twD + '</span><span>' + tcH + '</span><span>' + twH + '</span><span>' + tTIME + '</span></div>');
                            continue;
                        }
                        if (CountTC == 10) {
                            CountTC = 1;
        ...
        complete: function (XHR, TS) {
            ...
            if(Tag){
                SetTimeout = self.setTimeout(function(){
                    if($("#left_5").length==0&&$("#right_8").length==0&&$("#right_9").length==0){
                        //alert(1)
                    }
                    else{
                        Table589();
...
var Refresh_2 = () => {
    ...
    var timeRange = getQueryTimeRange();
    Stime2 = timeRange.Stime + " " + Stime;  // Stime此时还是LocalConfig的值，如"08:00:00"
    Etime2 = getCurrentDateTime();  // Etime此时还是LocalConfig的值，如"08:00:00"
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(data.EfficiencyOutput.length>0){
                for(var i=0;i<data.EfficiencyOutput.length;i++){
                    if (data.EfficiencyOutput[i].Name == "Connected") {
                        var Connect = data.EfficiencyOutput[i].Value[0] 
                        Connect=formatNumber(Connect);
                    ...
                    if (data.EfficiencyOutput[i].Name == "UpTime") {
                        var Running = data.EfficiencyOutput[i].Value[0]
                    ...
                    if (data.EfficiencyOutput[i].Name == "Cutting") {
                        var Cutting = data.EfficiencyOutput[i].Value[0]
                        Cutting=formatNumber(Cutting)
                        $("#popup_3").append('<div class= "MainMachineS_1" >' +
                                        '<div class="MachineS_name">Cutting</div>' +
                                        '<div class="MachineS_Per" style="background: conic-gradient(#39b6ec 0%, #39b6ec ' + Cutting + '%, #1f4e6c ' + Cutting + '%, #1f4e6c 100%);"><div class="MachineS_Single">' + Cutting + '%</div></div>' +
                        '</div>')
                    }
                    if (data.EfficiencyOutput[i].Name == "Alarm") {
                        var Alarm = data.EfficiencyOutput[i].Value[0]
                    ...
                    if(data.UsageType[i].Name=="NCProgram"){
                        ...
                        for (let j = 0; j < data.UsageType[i].Value.length; j++) {
                            if (CountNC == 1) {
                                ...
                                $("#popup_box_NC_data").append("<div><span>" + NcName + "</span><span>" + (NcTime / 60).toFixed(1) + " min</span><span>" + Ncfreq + " freq</span></div>")
...
function Download(){
    $.ajax({
        url: "/ChartApi/use",      
        data:{
            ip: getSelectedIp("Download"), // 华兴 更改
            port: getSelectedPort("Download"), // 华兴 更改
            start:Stime,
            end:Etime
        },
        type: "get",          
...
function MachineModeStatus(Mode){
    var color="";
    var Type=getSelectedType("Left234");
    if(Type=="FANUC"){
        if(Mode=="MDI"){
            color = "rgba(153,153,153,0.1)";
        }
        ...
        if(Mode=="DIAGNOSTIC"){color = "rgba(153,153,153,0.1)";}
        else if(Mode=="IDLE"){color = "rgba(153,153,153,0.2)";}
        else if(Mode=="MANUAL"){color = "rgba(153,153,153,0.3)";}
        else if(Mode=="MANUALSETUP"){color = "rgba(153,153,153,0.4)";}
        else if(Mode=="AUTOPREP"){color = "rgba(153,153,153,0.5)";}
        else if(Mode=="AUTORUN"){color = "rgba(153,153,153,0.6)";}
        else if(Mode=="INTERRUPTCYCLE"){color = "rgba(153,153,153,0.7)";}
        else{
            color = "rgba(153,153,153,0)";
        }
    }
    else if(Type=="MITSUBISHI"){
        if(Mode=="MANUAL"){color = "rgba(153,153,153,1)";}
        else if(Mode=="AUTOMATIC"){color = "rgba(153,153,153,2)";}
        else{color = "rgba(153,153,153,0)";}
        ...
        else if(Mode=="STOPPED_BY_M0"){
            color = "rgba(153,153,153,0.5)";
        }
        else if(Mode=="STOPPED_WITH_CYCLE_STOP"){
            color = "rgba(153,153,153,0.6)";
        }
        else if(Mode=="STOPPED_IN_SINGLE_BLK_MODE"){
            color = "rgba(153,153,153,0.7)";
        }
        else if(Mode=="SYNTAX_CHECKS"){
            color = "rgba(153,153,153,0.8)";
        }
        else if(Mode=="BLK_SEARCH"){
            color = "rgba(153,153,153,0.9)";
        }
        else if(Mode=="BLK_SEARCH_FINISHED"){
            color = "#000000";
        }
        else if(Mode=="EXE_TIME_EST"){
            color = "rgba(153,153,153,0)";
        }
        else if(Mode="IN_SIM"){
            color = "rgba(153,153,153,0)";
        ...
        if(Mode="EDIT"){color = "rgba(153,153,153,0.1)";}
        else if(Mode="MEM"){color = "rgba(153,153,153,0.2)";}
        else if(Mode="MDI"){color = "rgba(153,153,153,0.3)";}
        else if(Mode="DNC"){color = "rgba(153,153,153,0.4)";}
        else if(Mode="JOB"){color = "rgba(153,153,153,0.5)";}
        else if(Mode="HANDLE"){color = "rgba(153,153,153,0.6)";}
        else if(Mode="REF"){color = "rgba(153,153,153,0.7)";}
        else{
            color = "rgba(153,153,153,0)";
        }
    }
...
function JumpWeb(Type){
    if(Type=="0"){
        ...
        window.open("Part.html", "_self");
         ...
         window.open("ThreeColor.html", "_self");
    }
}
